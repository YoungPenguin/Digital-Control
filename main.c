...
//
//  The PI-lead control algorithm is the tustin implementation
//
 *------------------------------------------------------------------------------
 * Copyright (c) 2020
 * All rights reserved.
...
interrupt void adc_isr(void) {
    ...
    // PI-lead Tustiun implementation
    error     = Ref_v-adcresult;
    previous_error = error;
 
    error = error * kp;
 
    float integral = f1 * error + previous_error * f2 + previous_I;
    float derivative = integral * K1 + previous_I * K2 - K3 * previous_D;
    PI_output = derivative;
